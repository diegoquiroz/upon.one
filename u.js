    ...
    function litHasLoaded(module){
      ...
      class databaseSection extends LitElement {
          ...
          firstUpdated(){
            ...
            if(this.collections[0]) this.activeCollection = this.collections[0]
            
          }

        
          setActiveCollection(e){
            this.activeCollection = ''//this forces an update even if user has clicked on the same collection
            this.activeCollection = e.target.dataset.collection
          }

          checkActive(value,type){
            if(!type) type = 'Tab'
            if(value.toLowerCase() === this['active'+type]) return 'highlighted'
            return ''
          }
          ...
          render(){
            ...
            <div class='section'>
              ...
              <div class='left-pannel'>
                  ...
                  ${this.collections.map((item)=>html`
                    <button data-collection=${item} class='${this.checkActive(item,"Collection")}'  @click='${this.setActiveCollection.bind(this)}' >${item}</button>
                  `)}
              ...
              <div class='right-pad'>
                ...
                <documents-section .collectionName="${this.activeCollection}" ></documents-section>
            ...
            //change to schema and permission

            //top pannel: actions
            //Left pannel: collections
      ...
      customElements.define('database-section',databaseSection)
      ...
      class indivisualDocument extends LitElement{
        ...
        toggleEdit(event,updatedDoc){
          if(updatedDoc) this.document = {...updatedDoc}
        ...
        render(){
          ...
          function printAllFields(item){

            let allFields = []

            for(let key in item){
              if(key == 'id') continue
              allFields.push(html` <div> <b>${key}</b><br> <span> ${item[key]}<span> </div>`)
            }
          ...
          ${this.editMode? html`
            ...
            <add-document .toggleEditMode=${this.toggleEdit.bind(this)} .type="${`update`}" .collectionName=${this.collectionName} .document=${this.document} >
            </add-document>
          
          
          `:html`

            ${printAllFields(this.document)}

            <div id='buttonContainer'>
              <button @click=${this.toggleEdit.bind(this)} >Edit</button>
              <button @click=${this.deleteDocument.bind(this)} >Delete</button>
            </div>
      ...
      customElements.define('indivisual-document',indivisualDocument)
      ...
      class addDocument extends LitElement{
        ...
        firstUpdated(){
          ...
          for(let key in this.document){ //for making it easier and better suited for renaming fields

            if(key=='id') this.keyId = this.document[key]
            if(key!=='id') newDocument.push([key,this.document[key] ])
        ...
        updateDoc(event){
          ...
          let originalFieldName = event.target.getAttribute('data-field')
          let fieldIndex = event.target.getAttribute('data-fieldIndex')
          let value = event.target.value

          fieldIndex = Number(fieldIndex)

          if(event.target.classList.contains('field') ){
            this.document[fieldIndex][0] = value
          }else if(event.target.classList.contains('value')){
            this.document[fieldIndex][1] = value
          }
        }
        ...
        async save(){
          ...
          for(let index of this.document){
            if(index[0]) transformedDoc[index[0]] = index[1]
          }

          let baseQuery = {on:this.collectionName, put:transformedDoc  }

          let query = {$write: baseQuery}

          if(this.type == 'update'){
            baseQuery.where = {id:this.keyId}
          ...
          if(this.type == 'update'){
            return this.toggleEditMode(null,newDoc)
          }else{
            this.refresh()
            return this.toggleEditMode(null,newDoc)
          }
        ...
        render(){
          ...
          if( !Array.isArray(this.document) ) return html`nothing here`
          ...
          ${this.document.map(index=>{
            ...
            return html`
            <input placeholder="field" data-fieldIndex="${i}" @keyup="${this.updateDoc.bind(this)}" class="field" value="${key}" >
            <input placeholder="value" data-fieldIndex="${i}" @keyup="${this.updateDoc.bind(this)}" class="value" value="${value}" > <br><br>`
          ...
          <div id='buttonContainer'>
            <button  @click=${this.addField.bind(this)}>+ field</button> 
            <button  @click=${this.save.bind(this)}>${this.type}</button>
          </div>
          `
        }
      ...
      customElements.define('add-document',addDocument)
      ...
      class documentsSection extends LitElement{

        static get properties(){
          return {
            documents:Object,
            loading:Boolean,
            addDocumentMode:Boolean,
            addSchemaMode:Boolean,
            collectionName:String,
          }
        }
        ...
        toggleEditMode(){
          if(!this.addDocumentMode) return this.addDocumentMode = true
          this.addDocumentMode = false
          this.refresh()
        }
        ...
        updated(changedProperties){
          ...
          changedProperties.forEach((oldValue, propName) => {
            console.log(propName,propName == 'collectionName')
            if(propName != 'collectionName') return
            this.loading = true
        ...
        render(){
          ...
          if(!this.collectionName) return html``
          ...
          return html`
            ...
            ${this.addDocumentMode? html`<p> <add-document .refresh="${this.refresh.bind(this)}" .toggleEditMode=${this.toggleEditMode.bind(this)} .collectionName="${this.collectionName}"> </add-document> </p>`:html``}
            

            ${this.editSchemaMode? html`<schema-section></schema-section>`:html``}

            ${this.loading? html` <h1>Loading...</h1>`:html``}
            ${this.documents.length == 0 && this.loading == false? html` <h1>Nothing here</h1>`:html``}
           
            ${this.documents.map(item=>{ return html`

              <indivisual-document .document=${item} .collectionName=${this.collectionName}> </indivisual-document>
